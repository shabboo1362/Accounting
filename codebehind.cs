...
namespace Accounting
{
	...
	{	
		public SqlConnection connection=new SqlConnection("initial catalog=Test;integrated security=SSPI;persist security info=False;workstation id=SINDIA;packet size=4096;database=acconting1");
		private string str;
...
//		}
		...
		public void get_Info(string field,string table,string constraint,DataSet dataset,string tb)
		{
			string str="SELECT "+field+" FROM "+table+" WHERE "+constraint+";";
			select=new SqlCommand(str,connection);
		...
		public void datasetupdate(DataSet dataset,string table){
			dataadapter=new SqlDataAdapter();
			try
			{
				connection.Open();
